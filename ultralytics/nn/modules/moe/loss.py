...
class MoELoss(nn.Module):
    """Advanced Auxiliary losses for MoE models. Features: - Distributed-aware calculation - Support for both Hard
    (GShard-style) and Soft (Differentiable) load balancing - Entropy regularization to prevent router
    indecisiveness - Detailed diagnostic outputs.
    """
    ...
    def forward(
        ...
        # ------------------------------------------------------------------
        total_loss = (
            (self.balance_loss_coeff * balance_loss)
        ...
        # # 各部分损失打印
        # if self.training:
        #     print(f"[MoELoss] balance_loss: {balance_loss.item():.6f} "
        #           f"(coeff={self.balance_loss_coeff:.4f})")
        #     print(f"[MoELoss] z_loss:       {z_loss.item():.6f} "
        #           f"(coeff={self.z_loss_coeff:.4f})")
        #     print(f"[MoELoss] entropy_loss: {entropy_loss.item():.6f} "
        #           f"(coeff={self.entropy_loss_coeff:.4f})")
        #     print(f"[MoELoss] total_aux_loss: {total_loss.item():.6f}")
