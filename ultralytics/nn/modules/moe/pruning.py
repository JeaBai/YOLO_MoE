...
class MoEPruner:
    ...
    def _diagnose_usage(self) -> None:
        ...
        with ExpertUsageTracker(self.model.model) as tracker:
            try:
                self.model.val(data=self.dataset, split="val", batch=1, verbose=False, device="cpu")
    ...
    def _create_pruning_plan(self) -> None:
        ...
        for layer_name, stats in self.usage_stats.items():
            ...
            if len(experts_to_keep) == 0:
                print("     ❌ Error: All experts would be pruned! Keeping top expert.")
    ...
    def _find_projection_layer(self, router: nn.Module, num_experts: int) -> tuple[nn.Module, str] | None:
    ...
    def _prune_router_weights(self, router: nn.Module, keep_indices: list[int], num_old_experts: int) -> bool:
        ...
        if result is None:
            print("     ⚠️  Could not locate router projection layer. Skipping weight pruning.")
            return False
        ...
        if isinstance(proj_layer, nn.Conv2d):
            ...
            new_proj = nn.Linear(
                in_features=proj_layer.in_features, out_features=len(keep_indices), bias=(proj_layer.bias is not None)
            )
        else:
            return False
    ...
    def _perform_surgery(self) -> nn.Module:
        ...
        for layer_name, keep_indices in self.pruning_plan.items():
            ...
            print(f"     Experts: {num_old_experts} → {len(keep_indices)} (keeping {keep_indices})")

            # Prune experts
            self._prune_experts(moe_module, keep_indices)

            # Prune router weights
            self._prune_router_weights(moe_module.routing, keep_indices, num_old_experts)
    ...
    def _verify_model(self, output_path: str) -> bool:
        ...
        try:
            ...
            pruned_model.val(data=self.dataset, split="val", batch=1, verbose=False, device="cpu")
    ...
    def prune(self, output_path: str) -> bool:
        ...
        """
        print(f"\n{'=' * 80}")
        print("✂️  MoE MODEL PRUNING PIPELINE".center(80))
        print(f"{'=' * 80}")
        print("\n📋 Configuration:")
        ...
        try:
            ...
            if success:
                print(f"\n{'=' * 80}")
                print("🎉 PRUNING COMPLETED SUCCESSFULLY".center(80))
                print(f"{'=' * 80}\n")
...
def prune_moe_model(model_path: str, output_path: str, threshold: float = 0.15, dataset: str = "coco8.yaml") -> bool:
...
def main():
    ...
    success = prune_moe_model(args.model_path, args.output, args.threshold, args.dataset)
