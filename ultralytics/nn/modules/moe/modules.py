...
class UltimateOptimizedMoE(nn.Module):
    """Implicit MoE baseline: black-box router + shared expert + sparse experts.
    
    Uses only the router (no explicit descriptor) for implicit allocation.
    Compatible with yolo26_moe.yaml configuration.
    Args: [in_channels, num_experts, top_k, balance_coeff]
    """
    def __init__(self, in_channels, out_channels=None, num_experts=4, top_k=2,
                 balance_coeff=0.5, num_groups=4, **kwargs):
        super().__init__()
        ...
        self.shared_expert = InvertedResidualExpert(in_channels, out_channels, num_groups)
        self.experts = nn.ModuleList([
            InvertedResidualExpert(in_channels, out_channels, num_groups)
            for _ in range(num_experts)
        ])
        self.router = UltraEfficientRouter(in_channels, num_experts, top_k)
    ...
    def forward(self, x):
        ...
        # Sparse experts (simplified: process per-expert)
        experts_out = torch.zeros(B, C, H, W, device=x.device, dtype=x.dtype)
        for e in range(self.num_experts):
            e_mask = (routing_indices == e)  # [B, top_k, 1, 1]
            if e_mask.any():
                sel = e_mask.nonzero(as_tuple=False)  # [N, 4] -> [batch_idx, topk_idx, 1, 1]
                for row in sel:
                    bi = row[0].item()
                    ki = row[1].item()
                    expert_input = x[bi:bi+1]  # [1, C, H, W]
                    expert_output = self.experts[e](expert_input)  # [1, C, H, W]
                    weight = routing_weights[bi, ki, 0, 0]
                    experts_out[bi] += weight * expert_output[0]
        
        # Fusion gate
        gate_weight = self.fusion(x).unsqueeze(-1).unsqueeze(-1)  # [B, 2, 1, 1]
        shared_w = gate_weight[:, 0:1, :, :]
        sparse_w = gate_weight[:, 1:2, :, :]
        out = shared_w * shared_out + sparse_w * experts_out + identity
        
        # Metrics
        with torch.no_grad():
            top_k_per_sample = torch.full((B,), self.top_k, device=x.device)
            for e in range(self.num_experts):
                self._expert_hits[e] += (routing_indices == e).sum().float()
            self._token_count += routing_indices.numel()
            self._topk_sum += top_k_per_sample.float().sum()
            self._topk_count += B
    ...
    def get_metrics(self):
        if self._token_count.item() < 1:
            return {}
        hits = self._expert_hits / self._token_count
        ideal = 1.0 / self.num_experts
        return {
            'expert_usage': {f'e{i}': hits[i].item() for i in range(self.num_experts)},
            'max_usage': hits.max().item(),
